...
<a href='slotsmachine.php?tresde=$tresder'>&gt; Back</a></div><br></div></div></div></div></div>");
}
...
$won=$_GET['bet']*26;
$gain=$_GET['bet']*25;
print "and won \$$won by lining up 3 numbers pocketing you \$$gain extra.</div><br></div></div></div></div></div>";
}
else if ($slot[1]==$slot[2] || $slot[2]==$slot[3] || $slot[1]==$slot[3])
{
$won=$_GET['bet']*3;
$gain=$_GET['bet']*2;
print "and won \$$won by lining up 2 numbers pocketing you \$$gain extra.</div><br></div></div></div></div></div>";
}
else
{
$won=0;
...
<b><a href='explore.php'> Enough's enough, I'm off.</a></b></div><br></div></div></div></div></div>";
}
else
{
