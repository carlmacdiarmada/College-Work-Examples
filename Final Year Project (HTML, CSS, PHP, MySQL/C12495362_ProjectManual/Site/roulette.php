...
<a href='roulette.php?tresde=$tresder'>&gt; Back</a></div><br></div></div></div></div></div> ");
}
else if($_GET['bet'] > $maxbet)
{
die("You have gone over the max bet.<br />
<a href='roulette.php?tresde=$tresder'>&gt; Back</div><br></div></div></div></div></div> </a>");
}
...
$won=$_GET['bet']*37;
$gain=$_GET['bet']*36;
print "and won \$$won by matching the number u bet pocketing you \$$gain extra.</div><br></div></div></div></div></div> ";
}
else
{
$won=0;
$gain=-$_GET['bet'];
print "and lost it.</div><br></div></div></div></div></div> ";
}
