...
if($db->num_rows($qc))
{
print "You cannot add the same person twice.</div><br></div></div></div></div></div>";
}
else if($userid==$_POST['ID'])
{
print "You cannot be so lonely that you have to try and add yourself.</div><br></div></div></div></div></div>";
}
else if($db->num_rows($q)==0)
{
print "Oh no, you're trying to add a ghost.</div><br></div></div></div></div></div>";
}
else
{
...
<a href='friendslist.php'>&gt; Back</a></div><br></div></div></div></div></div>";
}
}
else
{
...
<input type='submit' STYLE='color: black;  background-color: white;' value='Add Friend' /></form></div><br></div></div></div></div></div>";
}
...
<input type='submit' STYLE='color: black;  background-color: white;' value='Change Comment' /></form></div><br></div></div></div></div></div>";
}
else
{
