...
echo '  <div class="nosign_line"></div>
...
{
echo '<img src="" alt="'.stripslashes($r['busName']).' banner" width="400" height="100" />';
}
      echo '</td></tr>
...
echo '</table></table></div><br></div></div></div></div></div> ';
}
...
<input type="submit" value="Donate Cash"></form></div><br></div></div></div></div></div>';
}
...
<a href="business_home.php?p=leave_business&confirm=1">Leave the Company</a></div><br></div></div></div></div></div>';
}
else
{
