...
<h2 style="padding-top:10px;"> Stock Market</h2>
...
<h2 style="padding-top:10px;"> Stock Market  - <a href="staff_stocks.php">Stock Panel </a> </h2>
';
...
function sell_stock()    {
    ...
    if(mysql_num_rows($holding) == 0)    {
        ...
        echo 'You sold '.number_format($soc['holdingQTY']).' stock of '.$stock['stockNAME'].' for '.money_formatter($tot).'! </div><div></div><br></div></div></div></div></div> ';
    }
}
function view_stock()    {
    global $ir,$h;
    ...
    if(mysql_num_rows($holdings) == 0)    {
        echo 'You do not hold any shares in any stocks at the moment, why not try investing?.</div><div></div><br></div></div></div></div></div>';
    }
    else    {
        echo '
...
function buy_stock()    {
    ...
    if(isset($_POST['buy']))    {
        ...
        if($ir['money'] <= $mprice)    {
            echo 'You do not have enough money for this amount of shares.';
            exit($h->endpage());
        }
        ...
        echo 'You successfuly bought '.number_format($amnt).' stocks of '.$stock['stockNAME'].'. </div><div></div><br></div></div></div></div></div>';
    }
    else    {
...
function stock_default()    {
    global $ir,$h;
    echo '
    Here is a list of available stock along with their price and changes<br />
    The Stock Market updates every 5 minutes !<br />
