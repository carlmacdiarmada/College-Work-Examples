...
<br />
<input type='submit' STYLE='color: black;  background-color: white;' value='Create User' /></form></div> <br></div></div></div></div></div>";}
...
$brave=3+$level*2;
$hp=50+$level*50;
$db->query("INSERT INTO users (username, login_name, userpass, level, money, crystals, user_level, energy, maxenergy, will, maxwill, brave, maxbrave, hp, maxhp, location, gender, signedup, email, bankmoney) VALUES( '{$_POST['username']}', '{$_POST['login_name']}', md5('{$_POST['userpass']}'), $level, $money, $crystals, $ulevel, $energy, $energy, 100, 100, $brave, $brave, $hp, $hp, 1, '{$_POST['gender']}', unix_timestamp(), '{$_POST['email']}', -1)");
$i=mysql_insert_id($c);
...
<input type='submit' STYLE='color: black;  background-color: white;' value='Edit User' /></form></div> <br></div></div></div></div></div>";}
...
<div class='generalinfo_simple'><br> <br><br>
      ...
      <form action='staff_users.php?action=deluser&step=2' method='post'>
      User: <input type='text' STYLE='color: black;  background-color: white;' name='user' value='0' /><br />
      <input type='submit' STYLE='color: black;  background-color: white;' value='Delete User' /></form></div> <br></div></div></div></div></div>";break;
   case 2:
      $target = $_POST['user'];
...
<div class='generalinfo_simple'><br> <br><br>
      ...
      <input type='submit' STYLE='color: black;  background-color: white;' name='yesorno' value='No' onclick=\"window.location='staff_users.php?action=deluser';\" /></form></div> <br></div></div></div></div></div>";break;
   case 3:
      $target = $_POST['userid'];
      if (!is_numeric($target)) exit;
      if (in_array($target,$undeletable)) {
...
User: ".user_dropdown($c,'user')."<br />
<input type='submit' STYLE='color: black;  background-color: white;' value='View Inventory' /></form></div> <br></div></div></div></div></div>";}
...
<input type='submit' STYLE='color: black;  background-color: white;' value='Credit User' /></form></div> <br></div></div></div></div></div>";}
...
<input type='submit' STYLE='color: black;  background-color: white;' value='Force User to Logout' /></form></div> <br></div></div></div></div></div>";}
}
